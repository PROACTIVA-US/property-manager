  ...
  if (compact) {
    return (
      <div className="space-y-3">
        <h4 className="text-sm font-bold text-brand-muted uppercase tracking-wider">Recent Payments</h4>
        {displayPayments.map((payment) => (
          <div key={payment.id} className="flex items-center justify-between py-2 border-b border-slate-700/50 last:border-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-brand-light">{formatShortDate(payment.date)}</span>
            </div>
            <span className="text-sm font-medium text-brand-light">{formatCurrency(payment.amount)}</span>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-brand-light">Rent Payments</h1>
          <p className="text-brand-muted mt-1">View payment history and make payments</p>
        </div>
      </div>
      ...
      <div className={`card ${
        balance.status === 'paid' ? 'bg-gradient-to-br from-green-900/40 to-brand-navy border-green-500/30' :
        balance.status === 'overdue' ? 'bg-gradient-to-br from-red-900/40 to-brand-navy border-red-500/30' :
        'bg-gradient-to-br from-yellow-900/40 to-brand-navy border-yellow-500/30'
      }`}>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="text-brand-muted mt-2">Due: {formatDate(balance.dueDate)}</p>
          </div>
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          {displayPayments.map((payment) => (
            <div
              key={payment.id}
              className="flex items-center justify-between p-4 bg-brand-dark/50 rounded-lg border border-slate-700/50 hover:border-slate-600 transition-colors"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-medium text-brand-light">{formatDate(payment.date)}</p>
              ...
              <div className="text-right">
                <p className="font-bold text-brand-light">{formatCurrency(payment.amount)}</p>
        ...
        {payments.length > 6 && (
          <button
            ...
            className="w-full mt-4 flex items-center justify-center gap-2 py-3 text-brand-muted hover:text-brand-light transition-colors"
          >
            {showAll ? (
              <>
                <ChevronUp size={20} />
