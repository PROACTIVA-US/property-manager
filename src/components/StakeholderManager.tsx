    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {project.stakeholders.map(stakeholder => (
          <div
            key={stakeholder.id}
            className="bg-brand-dark rounded-lg p-4 border border-white/10"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3 flex-1 min-w-0">
                <div className="flex-shrink-0 w-12 h-12 rounded-full bg-brand-darker flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <span className="text-xs text-brand-muted">Notifications:</span>
                        ...
                        <button
                          ...
                          className={`px-2 py-1 rounded text-xs transition-colors ${
                            stakeholder.notificationPreference === pref
                              ...
                              : 'bg-white/10 text-brand-muted hover:text-brand-light'
                          }`}
                        >
                          {pref === 'all' && 'All'}
        ...
        {project.stakeholders.length === 0 && !isAdding && (
          <div className="text-center py-8 bg-brand-dark rounded-lg border border-white/10">
            <User size={32} className="mx-auto text-brand-muted mb-2" />
            <p className="text-brand-muted">No stakeholders added yet</p>
          </div>
        )}
      </div>
      ...
      {isAdding ? (
        <div className="bg-brand-dark rounded-lg p-4 border border-white/10">
          <h4 className="font-semibold text-brand-light mb-3">Add Stakeholder</h4>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm text-brand-muted mb-1">Phone (Optional)</label>
                <input
                  type="tel"
            ...
            <div>
              <label className="block text-sm text-brand-muted mb-1">Notification Preference</label>
              <select
